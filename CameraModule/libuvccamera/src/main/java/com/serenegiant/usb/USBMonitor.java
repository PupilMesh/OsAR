...
public final class USBMonitor {
	...
	public final boolean hasPermission(final UsbDevice device) throws IllegalStateException {
		if (destroyed) throw new IllegalStateException("already destroyed");
		Log.i(TAG,"Has permission Method called");
	...
	public synchronized boolean requestPermission(final UsbDevice device) {
		Log.i("USBMonitor","Request for permission");
	if (DEBUG) Log.v(TAG, "requestPermission:device=");
			...
			if (device != null) {
				if (mUsbManager.hasPermission(device)) {
					...
					try {
						...
						Log.i(TAG," i am called from requesting permission");
					} catch (final Exception e) {
						Log.w(TAG, e);
	...
	private final BroadcastReceiver mUsbReceiver = new BroadcastReceiver() {
		...
		public void onReceive(final Context context, final Intent intent) {
			...
			if (destroyed){
				Log.i(TAG,"Destroid Receive method called");
				return;
			}
