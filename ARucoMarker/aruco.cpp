...
Mat generateGrid(vector<Mat> images,int rows,int cols,int margin=50,int padding=10){
    int img_count = 0;
...
void openCameraAndDetectMarker(){
    ...
    namedWindow("camera", WINDOW_AUTOSIZE); // create a window to show the camera feed
    while (true) {
        Mat frame;
    ...
    imshow("camera", frame);
        if (waitKey(30) == 'q') { // wait for 30ms, and exit if 'q' is pressed
            break;
        }
    }
}
